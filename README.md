...
| `instruments.csv` | 名琴流传谱系（琴名·藏者·流转序·出处·**琴铭·题咏**），第三类权威实体 | 30 |
...
- **v1.4.1**：`instruments.csv` 增补 `琴铭`/`题咏` 两列并新增「文山琴」(徐啸三藏，传文天祥遗物)——绿绮台殉国琴歌、叶犹龙赎琴诗、飞泉白沙手迹琴铭、文山琴琴背七绝等据史料补入；站点名琴卡片改为富悬停(题咏/琴铭/出处)。
- **v1.4.0**：新增 `instruments.csv`——名琴流传谱系，作为继**人物**、**琴谱**之后的第三类权威实体（**器物**）。含绿绮台 7 手流转链（唐武德制→明武宗御琴→邝露抱琴殉国→叶犹龙百金赎→…→邓尔雅）及 20 余张名琴的藏者。站点 `/lingnan` 同步新增"名琴流传谱系"与"岭南琴派发迹史"两节。
